...
func (r *relabeler) fetchAndRelabel(u *url.URL) ([]*dto.MetricFamily, error) {
...
func (r *relabeler) relabel(labels []*dto.LabelPair) ([]*dto.LabelPair, error) {
	relabeledSets, err := retrieval.Relabel(toLabelSet(labels), r.relabelCfgs...)
	if err != nil {
		return nil, errors.Wrap(err, "could not relabel")
	}
